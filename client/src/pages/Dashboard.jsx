...
const Dashboard = () => {
  ...
  useEffect(() => {
    const loadStats = async () => {
      ...
      setError('');
      try {
        const { data } = await api.get('/tasks/stats', { params: filters });
        setStats(data);
      } catch (err) {
    ...
    <div>
      <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">Task health across projects and teammates.</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {statCards.map(({ key, label, icon: Icon, color }) => (
            <div key={key} className="rounded-lg bg-white p-5 shadow-sm ring-1 ring-slate-200">
              <div className="flex items-center justify-between">
                <p className="text-sm font-semibold text-slate-500">{label}</p>
                <Icon className={color} size={22} />
              </div>
              <p className="mt-4 text-4xl font-bold text-ink">{stats?.[key] ?? 0}</p>
            </div>
          ))}
        </div>
      )}
